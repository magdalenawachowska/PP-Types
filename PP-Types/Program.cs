...
//{
    ...
    void PrintGroups(string[] t, int perLine)
    {
        ...
        while (i < arrLenght)
        {
            if (i+1 == arrLenght)
            {
                Console.Write($"{t[i]}. \n");             // uzywam write zamiast writeline, aby pisalo nie zaczynajac od nowej linii
                Console.WriteLine("");
            }
            else if ((i+1) % perLine == 0)
            {
                Console.Write($"{t[i]}, \n");             // zamiast tego tam gdzie to konieczne dodaje \n 
            }
            else if ((i+1) % perLine != 0)
            {
                Console.Write($"{t[i]}, ");            
    ...
    void PrintColumns(string[] t, int perLine, int width)
    {
        ...
        {
            if ((i+1) % perLine != 0)                                  // przypadek kiedy nie idziemy do nowej linii tylko pozostajemy w tym samym wierszu
            {
                if (t[i].Length >= width)
                {
            ...
            else if ((i+1) % perLine == 0)                          // ostatni element danego wiersza
            {
                if (t[i].Length >= width)
                {
                    Console.Write($"{t[i].Substring(0, width)} \n");
...
*/
    ...
    static void Main(string[] args)
    {
        ...
        program.PrintGroups(program.names, 5);
        //program.PrintColumns(program.names, 5, 15);
    }
    */
